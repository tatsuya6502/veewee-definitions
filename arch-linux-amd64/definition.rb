...
iso_mirror = 'http://mirrors.kernel.org/archlinux/iso/2014.04.01'
...
Veewee::Definition.declare({
  :cpu_count   => '1',
  :memory_size => '1024',    # Chef (ffi gem) did not build with '512'.
  :disk_size   => '10140',
  :disk_format => 'VDI',
  :hostiocache => 'off',
  ...
  :boot_cmd_sequence => [
    ...
    "#{root_password}<Enter>",
    "#{root_password}<Enter>",
    "echo 'Server = http://mirrors.163.com/archlinux/$repo/os/$arch' >  /etc/pacman.d/mirrorlist<Enter>",
    'systemctl start sshd.service<Enter><Wait>',
  ],
  :ssh_login_timeout => '1000em 0',
  :ssh_user          => 'root',
  ...
  :params => {
    ...
    :PACMAN_REFLECTOR_ARGS => '--verbose -c "China" -n 5 --sort rate --save /etc/pacman.d/mirrorlist',
  }
})
